...
//      // 这个实现类是 MyBatis 在运行时自动生成的，能够执行接口中定义的 SQL 语句。
           ...
           Book book = mapper.selectOneBook("Living","Yu Hua"); // 方法一


           System.out.println(book.getName());

            // 调用 BookMapper 接口中的 selectOneBooks 方法执行查询操作，返回查询结果的列表。
            // 是的，selectAllBooks 方法需要在 BookMapper 的 XML 映射文件中定义。MyBatis 会根据这个映射文件中的配置，执行对应的 SQL 查询并映射结果。

            Book b = new Book(); // 方法二
            b.setName("Living");
            b.setAuthor("Yu Hua"); // 一个参数，把它们封装成对象，再传到方法里
            ...
            Book book2 = mapper.selectOneBook3("Living",b); //方法三
